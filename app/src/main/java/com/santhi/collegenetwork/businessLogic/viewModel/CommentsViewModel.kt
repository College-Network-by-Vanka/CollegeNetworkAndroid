...
class CommentsViewModel:ViewModel() {
    private val database = Firebase.database("https://college-network-f83f1-default-rtdb.asia-southeast1.firebasedatabase.app/").reference.child("comments")
    val dataList = MutableLiveData<List<PostModel>>()
    val isLoading = MutableLiveData<Boolean>()


    fun fetchDataFromDatabase(postId:String) {
        isLoading.value = true

        database.child(postId).addValueEventListener(object : ValueEventListener {
