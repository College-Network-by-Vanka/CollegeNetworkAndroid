...
class UploadActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Firebase.database("https://college-network-f83f1-default-rtdb.asia-southeast1.firebasedatabase.app/").reference.child("users").child(college).child(roomId).child(uid.toString()).setValue(userModel).addOnCompleteListener {
            if (it.isSuccessful){
